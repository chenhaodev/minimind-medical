...
_ZH_CLASSIFY_RULES = [
    ("judgment", "brief", re.compile(
        r"应该|需不需要|要不要|该不该|是否|能不能|可以吗|对吗|好吗|可行"
        r"|严重吗|安全吗|危险吗|有必要吗|能治好吗|会不会有后遗|有没有风险"
        r"|是不是|健康吗|会.*吗|(?<![什为怎])么[？?]"
    )),
    ("explanation", "detailed", re.compile(
        r"为什么|是什么|什么是|解释|说明|介绍|如何理解"
        r"|想了解|有哪些|什么症状|什么原因|什么作用|什么影响|什么表现"
        r"|什么危害|什么效果|什么副作用|什么病症|什么情况"
        r"|请问|请教|想问|多久|多少|什么疾病|什么病"
        r"|想要了解|想要知道|患有|诊断|出现了症状"
        r"|问题描述|主要症状|伴有"
    )),
    ("creative", "detailed", re.compile(
        r"写一|写个|帮我写|创作|生成一[篇首段]|写一篇|续写|改编|改写|扩写"
    )),
    ("plan", "detailed", re.compile(
        r"如何|怎么|怎样|步骤|方法|流程|怎么做|给定.*将|对以下|根据.*生成|将其"
        r"|请推荐|需要注意|告诉我|治疗方案|有什么药|什么药"
    )),
]
